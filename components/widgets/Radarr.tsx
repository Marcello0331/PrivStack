...
function errorText(data: RadarrData | null) {
  const messages: Record<string, string> = {
    auth_failed: 'Radarr API key was rejected.',
    not_configured: 'Configure Radarr in settings or this widget.',
    unreachable: 'Radarr is unreachable from the server.',
  };

  return messages[data?.error || ''] || data?.message || 'Unable to load Radarr.';
}
...
export default function RadarrWidget({ config }: { config?: Record<string, any> }) {
  ...
  const runAction = async (action: 'refresh' | 'search-missing' | 'remove-queue', queueId?: number) => {
    if (action === 'search-missing' && !confirm('Search for all missing Radarr movies?')) return;
    if (action === 'remove-queue' && !confirm('Remove this queue item from Radarr?')) return;

    setActionId(`${action}:${queueId || 'all'}`);

    try {
      const response = await fetch('/api/widgets/radarr/action', {
        method: 'POST',
  ...
  return (
    <div className="space-y-3 text-sm">
      <div className="grid grid-cols-3 gap-2">
        <Stat label="Movies" value={data?.movieCount || 0} />
        <Stat label="Missing" value={data?.missingCount || 0} tone="text-red-400" />
      ...
      {isAdmin && (
        <div className="flex gap-2">
          <ActionButton title="Refresh library" loading={actionId === 'refresh:all'} onClick={() => runAction('refresh')}>
            <RefreshCw size={13} />
            Refresh
          </ActionButton>
          ...
          <div key={item.id} className="glass-sm p-2 rounded-lg text-xs">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-gray-400">{item.monitored ? 'Monitored' : 'Unmonitored'} - {item.hasFile ? 'Downloaded' : 'Missing'}</p>
              </div>
            </div>
          </div>
        ))}
