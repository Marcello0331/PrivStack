...
export async function POST(request: NextRequest) {
  try {
    ...
    const config = resolveServarrConfig('radarr', body.connectionId ? String(body.connectionId) : null);
    if (!config) {
    ...
    try {
      if (action === 'refresh') {
        await postCommand(config, 'RefreshMovie');
      }

      if (action === 'search-missing') {
        await postCommand(config, 'MissingMoviesSearch');
      }
    ...
    console.error('Radarr action error:', error);
